...
| **Two-Layer Anchoring: Stellar Disks Obey Newton, Gas Disks Deviate** | 星系物理 | [10.5281/zenodo.22771462](https://doi.org/10.5281/zenodo.22771462) |
...
### 案例一：星系旋转曲线 — 单层锚定（宇宙尺度）

用锚定框架拟合 98 个 SPARC 星系，中位残差 9.14 km/s，**不依赖暗物质**。α 参数（幸存因子对密度的敏感度）与星系的气体占比显著相关（r² = 0.439）。

### 案例二：星系旋转曲线 — 双层锚定与质量-尺度耦合（宇宙尺度）

在 159 个 SPARC 星系上进一步分析，发现：

- **恒星盘服从纯牛顿引力**（k_star = 0.005 ± 0.005 /kpc，与 0 一致）
- **气体盘偏离牛顿引力**（k_gas = 0.244 ± 0.022 /kpc，特征长度 4.1 kpc）
- **气体层锚定强度由两个独立量驱动**：气体质量占比 w_gas 和 HI 盘半径 R_HI
- **双变量模型**：k = 0.127·w_gas + 0.530/R_HI，5 折交叉验证 R² = 0.62
- **THINGS 巡天独立验证**：14 个重叠星系，大质量星系 k 一致（差值 < 0.03）

这一结果提供了不引入暗物质的旋转曲线解释，且与 MOND 可区分——MOND 不区分物质类型和分布尺度。

### 案例三：阿尔泰山西段子流域（水文尺度，高敏感）
...
- **数据源**：SPARC 星系数据库、THINGS 巡天、CNRD v1.0、中国雪深长时间序列数据集、NASA POWER、USGS NWIS
- **计算环境**：Python (h5py, numpy, matplotlib, scipy, sklearn)
- **分析框架**：锚定框架（S, G锚, R）
